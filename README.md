...
- `src/api/` — các hàm gọi API (auth, services, Orderings) qua `fetch`, tự gắn Bearer token.
- `src/context/AuthContext.tsx` — quản lý trạng thái đăng nhập toàn app.
- `src/components/` — Navbar, ProtectedRoute, ServiceCard.
- `src/pages/` — Home, Services, Ordering (cần đăng nhập), Login, Register, MyOrderings (cần đăng nhập).
...
- `POST /Orderings` → `Ordering`
- `GET /Orderings/me` → `Ordering[]`
- `DELETE /Orderings/:id` → `204`
